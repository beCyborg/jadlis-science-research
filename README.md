...
|---|---|---|
| **Откуда взялось «доказано».** До самой работы никто не доходит: в ходу пересказ блога, который пересказал пресс-релиз. | Пересказывает те же вторичные тексты; до полного текста статьи не идёт, а источник называет не всегда. | Ходит за один прогон в PubMed, Europe PMC, Semantic Scholar, OpenAlex, ClinicalTrials.gov, Epistemonikos, CORE (диссертации, отчёты, репозиторные копии — то, что в журналы не попало) и по сайтам Cochrane, NICE и UpToDate, а потом добирает статьи по спискам цитирования — вперёд и назад. |
| **Отозванные статьи.** Отозванную работу цитируют годами после отзыва, и в пересказе этого не видно. | Отвечает по тому, что было в обучении: об отзыве, случившемся позже, не знает. | Каждый DOI проверяется в Crossref до синтеза — по четырём сигналам отзыва (включая обратный поиск уведомления об отзыве); отозванные не попадают в выводы и перечисляются отдельной строкой. |
| **Правильная ли ссылка.** Проверить нечем, кроме как открыть каждую и сверить руками. | Ссылка выглядит настоящей и ведёт не туда: DOI и название расходятся. | С Crossref сверяются три поля — заголовок, год (допуск в год: онлайн-первым и в номере) и фамилия первого автора; не сошлось — статья помечена как неверифицированная и в доказательную таблицу не идёт. |
...
| **Кто спорит с выводом.** Спорить некому: черновик читает тот, кто его и составил. | Соглашается сам с собой и добавляет уверенности при уточняющем вопросе. | Отдельный критик ищет опровержения, перепроверяет отзыв по ключевым DOI и смотрит PubPeer. Каждую работу, которой в подборке не хватило, критик обязан назвать отдельным списком — и она попадает в отчёт, а не теряется в разборе. |
| **Вопрос из двенадцати подтем.** Читать приходится по одной теме за раз, и половина вопросов остаётся без ответа. | Отвечает на главную тему, боковые подвопросы («а на сон?», «а кофеин вместо этого?») просто не ищет. | Вопрос сначала раскладывается на подвопросы, и каждый получает собственный короткий запрос к базам. В отчёте видно, сколько статей нашлось по каждому: ноль — значит дыра в данных, и так и написано. |
| **Отчёт написан для учёных.** Метки вроде «g 0,34» и «OR 2,82 (95% CI …)» приходится расшифровывать самому. | Пишет либо тем же языком, либо теряет оговорки при упрощении. | Видимая часть отчёта — простым языком: уверенность словами, ссылки сносками, размер эффекта бытовым сравнением. Таблицы с кодами и интервалами остаются внизу под свёрткой — для проверки. |
...
Отчёт читается без словаря: «реакция становится немного лучше, уверенность средняя», а не
«g 0,34 [em1·MODERATE]». Коды статей, GRADE и доверительные интервалы живут в свёрнутых таблицах
внизу — там, где их и проверяют.
...
5. /jadlis-search:keys — заведи ключи научных источников: PubMed, Semantic Scholar, OpenAlex,
   CORE и почты для Crossref и Unpaywall.
...
Scholar, OpenAlex, CORE и контактные почты для Crossref и Unpaywall: они заводятся один раз командой
