...
- Вызовы ниже **независимы — отправь их одним сообщением, в параллель** (общий набор + набор под
  свою дисциплину), а не по одному. Тариф Search держит 50 req/s, ждать между ними нечего.
- Не скрейпь найденные страницы поодиночке: `extra_snippets` Brave уже даёт материал на
  `Key claims`; полный текст статьи-обзора здесь не нужен — это контекст, не primary research.
- **Цель: ≤10 ходов** на источник (все вызовы в одном ходе → Read/оценка → Write дампа → ответ).
