...
У Epistemonikos нет стабильного открытого REST API для агентов — используем scrape страницы поиска. Загрузи firecrawl через ToolSearch (`select:mcp__plugin_jadlis-search_firecrawl__firecrawl_scrape`).

### Бюджет ходов

Скрипта у этого источника нет (только firecrawl/Brave через MCP), поэтому ходы экономятся
параллелизмом и жёстким потолком на scrape.

- Scrape страницы поиска — один вызов. Если за DOI нужно открыть карточки записей, **отправь эти
  вызовы одним сообщением, в параллель**, и не более 1–2 штук (суммарно 2–3 scrape, см. Rate limits).
- Не открывай карточку каждой найденной записи: DOI, которого не видно, остаётся `null` — это
  штатный исход, а не повод на лишний ход.
- **Цель: ≤10 ходов** на источник (scrape → Read/разбор карточек → добор DOI одним ходом →
  Write дампа → ответ).
...
1. Загрузи brave (`select:mcp__plugin_jadlis-search_brave-search__brave_web_search`).
2. Один вызов:
```
