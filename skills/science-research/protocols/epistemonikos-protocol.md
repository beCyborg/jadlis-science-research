...
У Epistemonikos нет стабильного открытого REST API для агентов — используем scrape страницы поиска. Загрузи firecrawl через ToolSearch (`select:mcp__plugin_search_firecrawl__firecrawl_scrape`).
...
1. Загрузи brave (`select:mcp__plugin_search_brave-search__brave_web_search`).
2. Один вызов:
```
