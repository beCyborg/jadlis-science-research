...
**Budget:** S2 REST ≤13 | Crossref curl ≤10 (+ ≤10 на резолв DOI шага 7.5) | Brave ≤10

---

## Железное правило: названа отсутствующей → в `missingPapers[]`

Любая работа, про которую ты пишешь «в корпусе её нет», «отчёт её не увидел», «пропущена» — где бы
это ни было: в контраргументах, в таблице проверки claims, в оценке publication bias, в сверочном
шаге 7.5 — **обязана попасть в `missingPapers[]`** структурированного ответа: `doi` (резолвленный),
`pmid`, `title`, `why`, `targetSection`.

Почему отдельным массивом, а не строкой разбора: fix-агент читает только структурированный ответ и
таблицу правок. 21.09.2026 критик нашёл Kanobe 2025 и назвал её в таблице проверки (`adversarial.md:19`),
в `edits[]` она не попала — и в заметке оказалось ноль вхождений. Работа была потеряна дважды.

Перед возвратом ответа пройди по собственному `adversarial.md` и сверь каждое упоминание
отсутствующей работы со списком. В сам файл добавь секцию `### Пропущенные работы` тем же составом.
...
(см. ниже шаг 7.5 — сверочный добор по подтемам с дырой)
...
### Шаг 7.5. Сверочный добор по подтемам с дырой — Brave ≤5 ВСЕГО

Ядро передаёт в промпте список подтем, у которых в корпусе 0–1 статья (телеметрия `coverage`).
Список пуст → шаг выполняется, только если ты сам видишь непокрытую подтему.

1. На подтему — один запрос вида `«<подтема> systematic review»` или `«<подтема> cohort»`.
   Всего по всем подтемам — **не больше 5 вызовов Brave**.
2. Найденный DOI включается ТОЛЬКО после резолва (как и весь остальной корпус):
   ```bash
   curl -s -o /dev/null -w "%{http_code}" "https://api.crossref.org/works/{DOI}"      # 200 → ок
   curl -s -o /dev/null -w "%{http_code}" "https://api.datacite.org/dois/{DOI}"       # 200 → серая литература, тоже ок
   ```
   Не резолвится ни там, ни там → работы для отчёта не существует. **DOI не выдумывать** — выдача
   веб-поиска для того и проверяется, что она приносит и мусор, и несуществующие идентификаторы.
3. Что прошло резолв — в `missingPapers[]` с `why: "подтема <X> без статей в корпусе"`.
4. Подтема осталась пустой и после сверки → это честный gap: строкой в `### Gaps`, а не правкой.
...
### Пропущенные работы
Тот же состав, что в `missingPapers[]` ответа. Каждая строка — работа, которой в корпусе нет.

| # | Работа | DOI (резолвлен) | Чем меняет картину | Куда вписать |
|---|--------|-----------------|--------------------|--------------|
| M1 | {автор, год, название} | 10.x/y (Crossref / DataCite) | закрывает подтему «сперма» | Безопасность |
...
| Crossref/DataCite резолв | ≤10 | Шаг 7.5 (проверка найденных DOI) |
| Brave `brave_web_search` | ≤10 | Шаги 3.5 + 6 (≤5) + 7.5 (≤5) |
