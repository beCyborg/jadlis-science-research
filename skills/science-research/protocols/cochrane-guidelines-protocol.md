...
Скрипта у этого источника нет (Brave — MCP, не REST), поэтому ходы экономятся параллелизмом.

- Три вызова ниже **независимы — отправь их одним сообщением, в параллель**, а не по одному.
  Тариф Search держит 50 req/s, ждать между ними нечего.
- Не ходи по страницам выдачи поодиночке: ничего, кроме перечисленных в протоколе вызовов,
  скрейпить не нужно — сниппетов Brave хватает на `Key claims` и `Main conclusion`.
- **Цель: ≤10 ходов** на источник (3 вызова в одном ходе → Read/оценка → Write дампа → ответ).
  На 21.09.2026 этот источник был самым дорогим в fan-out: 52 хода.
