#!/usr/bin/env bash
# sync-shared.sh — refresh the vendored copy of scripts/ and shared/ from the jadlis-search plugin.
# Usage: bash tools/sync-shared.sh jadlis-search--v1.2.0
set -euo pipefail
TAG="${1:?usage: sync-shared.sh <jadlis-search--vX.Y.Z>}"
